...
class ReportProductCreateView(APIView):
    ...
    def post(self, request, product_id):
        product = get_object_or_404(Product.objects.alive(), pk=product_id)
...
class ReportVendorCreateView(APIView):
    ...
    def post(self, request, vendor_id):
        vendor_profile = get_object_or_404(VendorProfile.objects.alive(), pk=vendor_id)
        serializer = ReportCreateSerializer(data=request.data)
        serializer.is_valid(raise_exception=True)
...
class MyReportsListView(generics.ListAPIView):
    """GET /api/v1/reports/mine/ — reports filed by the authenticated user."""
...
class ReportDetailView(generics.RetrieveAPIView):
    """GET /api/v1/reports/{report_id}/ — the reporter (own report) or admin."""

    permission_classes = [IsAuthenticatedCustomer, IsReportOwnerOrAdmin]
...
class ReportUnderReviewView(APIView):
    ...
    def post(self, request, report_id):
        report = get_object_or_404(Report.objects.alive(), pk=report_id)
        report = ReportService.mark_under_review(report=report)
        return success_response(
